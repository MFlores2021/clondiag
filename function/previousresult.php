	...
	if ($rs = mysqli_query($conn, $sql)) {
		...
		while ($result = mysqli_fetch_assoc($rs)){
			...
			if ($result["test"]) {
				...
				$sql1 	= "SELECT `test`.`date` AS `fecha`, `test`.`name`, `test`.`owner`, `test`.`dir` AS 'testdir', `project`.`dir`  AS 'projdir', `project`.`file`  AS 'book', `project`.`array`  AS 'array' , `test`.`id` AS 'id', `project`.`name` AS 'projname' FROM `test` INNER JOIN `project` ON `test`.`project` = `project`.`id` WHERE `test`.`id` = '" . $result["test"] . "' ORDER BY `test`.`date` DESC;";
				...
				if ($rs1 = mysqli_query($conn, $sql1)) {
					...
					while ($result1 = mysqli_fetch_assoc($rs1)){
						...
						if (file_exists($dir_result)) {
							$testdir 	 =  $result1["testdir"];
							$prev		.=  "<tr><td width=\"15%\">" . date("Y-m-d", strtotime($result1["fecha"])) . "</td><td width=\"40%\"><a href =\"step/C1.php?testdir=$testdir&user=$user&dir_result=/clondiag/$dir_result\">" . $result1['name'] . "</a>";
						}
						else {
							$projdir	 =  $result1["projdir"];
							$testdir 	 =  $result1["testdir"];
							...
							$prev		.=  "<tr><td width=\"17%\">" . date("Y-m-d", strtotime($result1["fecha"])) . "</td><td width=\"40%\"><a href =\"step/A1.php?projdir=$projdir&testdir=$testdir&book=$book&user=$user&array=$array\">" . $result1["name"] . "</a>";				
						}			
												
						$prev		.=  "</td><td width=\"15%\">" . $result1["owner"] . "</td><td width=\"20%\">" . $result1["projname"] . "</td>";
						...
						if ($result1["owner"] == $user) {
							...
							if(strlen($result1["testdir"]) > 0){
								...
								$prev		.=  "<td width=\"10%\"><form name='form1' method='POST' action=''><input type='submit'  name='delete' id='delete' value='Delete' onclick='return confirm(\"Are you sure you want to delete " . $result1["testdir"] . " test?\")'><input type='hidden' id='id' name='id' value='" . $result1["id"] . "' /><input type='hidden' id='dir' name='dir' value='" . $dir_del . "' /></form></td></tr>";	
